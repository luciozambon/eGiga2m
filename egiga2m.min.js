...
function plotData(a,d,g,h){g=g.split(";");h=h.split(";");document.getElementById("show_hc").checked?(hcPlot(a,d,g,h),document.getElementById("pngCallback").style.display="none"):document.getElementById("show_chartjs").checked?(chartjsPlot(a,d,g,h),document.getElementById("pngCallback").style.display="inline"):document.getElementById("show_flot").checked?(flotPlot(a,d,g,h),document.getElementById("pngCallback").style.display="inline"):($("#mybackground").height("1px"),$("#placeholder").height("1px"),
...
function chartjsPlot(a,d,g,h){d=[window.chartColors.blue,window.chartColors.red,window.chartColors.green,window.chartColors.orange,window.chartColors.magenta,window.chartColors.brown];var e=document.getElementById("minY")&&document.getElementById("minY").value.length?document.getElementById("minY").value.split(";"):[],b=document.getElementById("maxY")&&document.getElementById("maxY").value.length?document.getElementById("maxY").value.split(";"):[],k=document.getElementById("logY")&&document.getElementById("logY").value.length?
document.getElementById("logY").value.split(";"):[];g=Chart.helpers.color;h={legend:{position:"bottom"},radius:20,scales:{xAxes:[{type:"time",time:{format:"DD/MM/Y HH:mm:ss",tooltipFormat:"DD/MM/Y HH:mm:ss"},scaleLabel:{display:!0,labelString:"Date"},ticks:{callback:function(q,v,w){return v==w.length-1?"":moment(w[v]?w[v]._i:0).format("DD/MM/Y")}}}],yAxes:[{id:"y1",type:"linear",position:"left",ticks:{}},{id:"y2",type:"linear",position:"right",grid:!1,ticks:{}}]}};""!=e[0]&&(h.scales.yAxes[0].ticks.min=
e[0]-0);""!=b[0]&&(h.scales.yAxes[0].ticks.max=b[0]-0);"0"!=k[0]&&(h.scales.yAxes[0].type="logarithmic");""!=e[1]&&(h.scales.yAxes[1].ticks.min=e[1]-0);""!=b[1]&&(h.scales.yAxes[1].ticks.max=b[1]-0);"0"!=k[1]&&(h.scales.yAxes[1].type="logarithmic");e=[];b=!1;for(k=0;k<a.length;k++){for(var l=[],m=0;m<a[k].data.length;m++)l.push({x:a[k].data[m][0],y:a[k].data[m][1]});"2"==a[k].yaxis&&(b=!0);e.push({label:a[k].label,backgroundColor:g(d[k%d.length]).alpha(.5).rgbString(),borderColor:d[k%d.length],fill:!1,
pointStyle:"line",data:l,yAxisID:"y"+a[k].yaxis})}b||h.scales.yAxes.pop();a={type:"line",data:{datasets:e},options:h};$("#canvas").show();$("#placeholder").hide();d=document.getElementById("canvas").getContext("2d");window.myLine=new Chart(d,a)}
