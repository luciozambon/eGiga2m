...
path[path.length-1]=-1==r.indexOf(".html")?path[path.length-1]+"index_plot.html":path[path.length-1].replace(".html","_plot.html");plotURL=window.location.protocol+"//"+window.location.host+path.join("/");path.pop();0==exportService.indexOf("./")&&(exportService=window.location.protocol+"//"+window.location.host+path.join("/")+exportService.substr(1));exportURL=exportService+g+l+b;var w=document.getElementById("downtimeCheck").checked?"&downtimeCheck=true":"",x="maxmin"!==decimation?"&decimation="+
decimation:"",u=1E3!=decimationSamples?"&decimationSamples="+decimationSamples:"",z=document.getElementById("hideOverMaxmin")&&document.getElementById("hideOverMaxmin").checked?"&hideOverMaxmin=true":"";r=e+g+l+b;h=h+n+q+x+u+w+z;jsonURL=window.location.protocol+"//"+window.location.host+path.join("/")+plotService.substr(1)+g+l+b+h;b=window.location.protocol+"//"+window.location.host+path.join("/")+treeService.substr(1)+g+l+b+h;if("undefined"!==typeof a){if("history"==a)return r+h+""+m+v;window.location=
p+"?"+e+g+l+"&ts="+a+h+m+v}else document.getElementById("plotOnly").setAttribute("onClick","javascript: location='"+plotURL+"?"+r+h+m+v+t+"'"),document.getElementById("plotOnly").innerHTML=plotURL+"?"+r+h+m+v+t,document.getElementById("plotAndControls").setAttribute("onClick","javascript: location='"+p+"?"+r+h+m+v+t+"'"),document.getElementById("plotAndControls").innerHTML=p+"?"+r+h+m+v+t,document.getElementById("scratch")&&(document.getElementById("scratch").setAttribute("onClick","javascript: location='"+
p+"?"+e+"'"),document.getElementById("scratch").innerHTML=p+"?"+e),exportBlanks=document.getElementById("exportZoh").checked?"&zoh":document.getElementById("exportFoh").checked?"&foh":"",exportFormat=document.getElementById("exportCsv").checked?"csv":"",exportFormat=document.getElementById("exportXls").checked?"xlsx":exportFormat,exportFormat=document.getElementById("exportMat").checked?"mat":exportFormat,exportFormat=document.getElementById("exportIgor").checked?"itx":exportFormat,exportFormat=document.getElementById("exportJson")&&
...
function switchtree(){if("<img src='./img/right.png'>"==document.getElementById("hidetree").InnerHTML){document.getElementById("hidetree").InnerHTML="<img src='./img/left.png'>";$("body").find("#hidetree").html("<img src='./img/left.png'>");document.getElementById("treeid").style.display="inline";var a=$(window).width()-280;300>a&&(a=300);$("#mybackground").width(a).height(height-0+10+"px");$("#placeholder").width(a-10).height(height+"px")}else document.getElementById("hidetree").InnerHTML="<img src='./img/right.png'>",
$("body").find("#hidetree").html("<img src='./img/right.png'>"),document.getElementById("treeid").style.display="none",a=$(window).width()-30,300>a&&(a=300),$("#mybackground").width(a).height(height-0+10+"px"),$("#placeholder").width(a-10).height(height+"px");rePlotTs()}
...
function plotTs(a,b,d){curves=[];myRequest=a;var g="";for(j=0;j<events.length;j++)g+=document.getElementById("show_"+events[j]).checked?"&show_"+events[j]+"="+document.getElementById("filter_"+events[j]).value:"";g=g+"&decimation="+decimation+"&decimation_samples="+decimationSamples;console.log(document.getElementById("hideOverMaxmin").checked);if(document.getElementById("hideOverMaxmin")&&document.getElementById("hideOverMaxmin").checked){g+="&hideOverMaxmin=";var l=document.getElementById("maxY").value;
1<l.length&&(g+="&maxY="+l);l=document.getElementById("minY").value;1<l.length&&(g+="&minY="+l)}l=document.getElementById("height").value.length?document.getElementById("height").value:$(window).height()-200;300>l&&(l=300);var e=$(window).width()-(0<$("#tree").length?280:0);300>e&&(e=300);$("#mybackground").width(e).height(l-0+10+"px");$("#placeholder").width(e-10).height(l+"px");$("#placeholder").html("&nbsp;&nbsp;&nbsp;&nbsp;<img src='./img/timer.gif'>");l="start="+b;e="";updatePlot=0;0<=d.indexOf("update")?
(e=d.split(","),updatePlot="undefined"===typeof e[1]?1:.1<e[1]?e[1]:.1,updateBackground="undefined"===typeof e[2]?300:60<e[2]?e[2]:60,e="&no_posttimer="):d.length&&(e="&stop="+d);var h=document.getElementById("show_hc").checked?"&prestart=hc":"";a=decodeTs(a);var m=d.length?new Date(d):new Date;$.get(plotService+"&Seconds_Behind_Master",function(a){60<a&&alert("WARNING\nThe data has not been updated for "+a+" seconds")});$.get(plotService+"&"+l+e+"&ts="+a+h+g,function(a){if(document.getElementById("downtimeCheck")&&
document.getElementById("downtimeCheck").checked){for(var e=a.ts[0].data[0].x?a.ts[0].data[0].x:a.ts[0].data[0][0],g=[],h=0;h<a.ts.length;h++)(m.valueOf()-a.ts[h].data[a.ts[h].data.length-1][0])/(m.valueOf()-e)>10/a.ts[h].data.length&&g.push(a.ts[h].label);g.length&&alert("WARNIG\nsome data may be missing (may be server or replication downtime) for Time Series:\n"+g.join(","))}plotData(evalFormulae(a.ts),a.event,b,d)})}
...
[],v=document.getElementById("logY")&&document.getElementById("logY").value.length?document.getElementById("logY").value.split(";"):[];document.getElementById("style")&&document.getElementById("style").value.length&&(h=document.getElementById("style").value);var r=1,t=1,p;for(p in curves)t<curves[p].y&&(t=curves[p].y);for(p=k=0;p<events.length;p++)document.getElementById("event_"+events[p]).style.display="none";for(p in curves)if("clone"!=p)if(a)for(;a[k]&&a[k].ts_id==curves[p].request;){myPlotClass[k]=
