		...
		foreach ($ts_array as $ts_num=>$ts_id_num) {
			...
			if ($pretimer and (mysqli_num_rows($res)<500)) {
				...
				if (mysqli_num_rows($res2)>0) {
					...
					if (round($row['time'])<strtotime($start[$xaxis-1]." $timezone"))
	...
	if ($show['error']) {
		...
		foreach ($ts as $xaxis=>$ts_array) {
			...
			foreach ($ts_array as $ts_num=>$ts_id_num) {
				...
				$col_name = 'error_desc';
				$orderby = $dim=='array'? "time,idx": "data_time";
				$filter = (strlen($_REQUEST["show_error"]) and ($_REQUEST["show_error"]!=='1'))? "$col_name LIKE ".quote_smart(strtr($_REQUEST["show_error"], array('*'=>'%'))): "NOT ISNULL($col_name)"; 
...
/*
	...
	if ($show['alarm']) {
		$data = array();
		$messages = array();
		$db = mysqli_connect('alarm-server', 'alarm-user', 'alarm-password');
		...
		$big_data['event']['alarm']['message'][0] = 'Fault Conditioning Mod. 7';
		$big_data['event']['alarm']['message'][1] = "Situazione anomala sul modulatore 3: trigger thyratron assente e klystron filament 100%. Se permane l'anomalia fra 15 minuti verra' spenta l'alta tensione e i filamenti klystron saranno posti al 80%";
		$big_data['event']['alarm']['message'][2] = 'BC01: radiation alarm ';
		$big_data['event']['alarm']['data'][0] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 4.1) * 1000, 'y'=>$y, 'message'=>0,'marker'=>array('symbol'=>"url($host/img/event_alarm.png)"));
		$big_data['event']['alarm']['data'][1] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 2.8) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_alarm.png)"));
		$big_data['event']['alarm']['data'][2] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 2.1) * 1000, 'y'=>$y, 'message'=>2,'marker'=>array('symbol'=>"url($host/img/event_alarm.png)"));
		$big_data['event']['alarm']['data'][3] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 1.2) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_alarm.png)"));
	}

	if ($show['command']) {
		$big_data['event']['command']['message'][0] = 'f/modulators/modulators=>ControlledAccess';
		$big_data['event']['command']['message'][1] = "kg02/mod/hv=>Off";
		$big_data['event']['command']['message'][2] = 'kg07/mod/modcond-kg07-01=>On';
		$big_data['event']['command']['data'][0] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 7.1) * 1000, 'y'=>$y, 'message'=>0,'marker'=>array('symbol'=>"url($host/img/event_command.png)"));
		$big_data['event']['command']['data'][1] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 3.8) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_command.png)"));
		$big_data['event']['command']['data'][2] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 2.5) * 1000, 'y'=>$y, 'message'=>2,'marker'=>array('symbol'=>"url($host/img/event_command.png)"));
		$big_data['event']['command']['data'][3] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 1.05) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_command.png)"));
	}
	if ($show['button']) {
		$big_data['event']['button']['message'][0] = 'modmulti - SwitchToControlledAccess';
		$big_data['event']['button']['message'][1] = "kg02 conditioning - Off";
		$big_data['event']['button']['message'][2] = 'kg07 control - On';
		$big_data['event']['button']['data'][0] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 7.1) * 1000, 'y'=>$y, 'message'=>0,'marker'=>array('symbol'=>"url($host/img/event_button.png)"));
		$big_data['event']['button']['data'][1] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 3.8) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_button.png)"));
		$big_data['event']['button']['data'][2] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 2.5) * 1000, 'y'=>$y, 'message'=>2,'marker'=>array('symbol'=>"url($host/img/event_button.png)"));
		$big_data['event']['button']['data'][3] = array('x'=>(strtotime($start[0]) + ($stop_timestamp[0] - strtotime($start[0])) / 1.05) * 1000, 'y'=>$y, 'message'=>1,'marker'=>array('symbol'=>"url($host/img/event_button.png)"));
	}
*/
	// debug($query); exit(0);
