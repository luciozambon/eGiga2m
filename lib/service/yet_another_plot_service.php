<?php
	...
	if (isset($_REQUEST["stop"]) and strlen($_REQUEST["stop"])) {
		...
		foreach ($stop as $k=>$val) {
			$time = parse_time($val);
			$stop[$k] = strlen($val)? " AND topupmsg_id <= (SELECT id FROM topupmsg WHERE timestamp <= unix_timestamp('$time') ORDER BY timestamp DESC LIMIT 1)": ' ';
	...
	foreach ($ts as $xaxis=>$ts_array) {
		...
		foreach ($ts_array as $ts_num=>$ts_id_num) {
			$big_data_w = array();
			$res = mysqli_query($db, "SELECT * FROM adt WHERE ID=".quote_smart($ts_id_num[0]));
			if (isset($_REQUEST['debug'])) echo "SELECT * FROM adt WHERE ID={$ts_id_num[0]};<br>\n";
			$row = mysqli_fetch_array($res, MYSQLI_ASSOC);
			$table = sprintf("topup_{$ts_id_num[0]}");
			...
			$qstart = "topupmsg_id >= (SELECT id FROM topupmsg WHERE timestamp >= unix_timestamp('{$start[$xaxis-1]}') ORDER BY timestamp LIMIT 1)";
			$query = "SELECT timestamp AS time, $col_name FROM $table,topupmsg WHERE topupmsg_id=topupmsg.id AND $qstart{$stop[$xaxis-1]} ORDER BY $orderby";
